...
test('species modal has a pose toggle and genus/rarity meta', async ({ page }) => {
  ...
  await expect(page.locator('#modalPoseToggle button')).toHaveCount(2);

  await page.click('#modalPoseToggle button[data-pose="1"]');
  await expect(page.locator('#modalPoseToggle button[data-pose="1"]')).toHaveAttribute('aria-current', 'true');
});
