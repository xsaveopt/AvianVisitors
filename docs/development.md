...
The app is Vue 3 + Vite + TypeScript in webui/frontend. There is one pnpm package
at webui/ that holds the app deps plus the oxlint/oxfmt and Playwright tooling.

```sh
cd webui
pnpm install
pnpm run lint            # oxlint frontend/src
pnpm run format          # oxfmt frontend/src/styles.css
